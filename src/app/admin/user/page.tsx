...
const Page = (props: Props) => {
    ...
    useEffect(() => {
        currentUser?.position && getItem(currentUser?.position, "user", search, page * limit, limit)
        currentUser?.position && getItemV2(currentUser?.position, "user", search, page + 1 * limit, limit)
    ...
    return (
        <div style={{ height: "100vh", width: "100%", padding: "0 10px" }}>
            <div style={{ width: "max-content", margin: "0" }}>
                <Button name='new user' onClick={() => toPage.push("post/new")} />
            </div>
            <div className='display-flex' style={{ height: "40px" }}>
                ...
                <div style={{ width: "calc(100% - 80px)", height: "100%", lineHeight: "50px", fontWeight: "bold", margin: "" }}>users</div>
            ...
            {data.length ? data.map((item, index) =>
                <div key={index} className='display-flex hover-background-color-128-15p hover-boder-radius-5px hover-opacity-1'
                    ...
                    <div style={{ width: "calc(100% - 80px)", height: "100%", lineHeight: "40px" }}>
                        <p onClick={() => toPage.push("user/" + item._id)}>
                            {item.username}
                ...
                <div className='flexbox'>
                    ...
                    <div style={{ width: "100%", textAlign: "center" }}>{loading ? <p>wait a minute</p> : <p>there is no users</p>}</div>
                    <div style={{ width: "50px" }}></div>
                </div>
            }
